...
Un **paquete** de R es una colección de funciones, datos y documentación, que extiende las capacidades iniciales de la base de `r R()`. Existen multitud de paquetes de `r R()`, desarrollados por la comunidad de usuarios de `r R()`, que añaden funciones  para propósitos específicos. 
...
La instalación de este paquete puede llevar unos minutos (hay que esperar hasta ver en la consola de `r RStudio()` que se ha completado el proceso).
...
Para cargar un paquete previamente instalado hay que usar la función `library()`. Por ejemplo, para cargar el paquete `tidyverse` escribimos
```{r}
library("tidyverse")
```
