...
las incluiremos en todos nuestros documentos R Markdown, y establecen la opción de mostrar el código que incluyamos en el cuerpo de nuestro documento \texttt{.Rmd} en el formato de salida.
...
Los elementos más importantes del documento son los bloques de código \textsf{R} (\emph{code chunks}). En el documento de muestra encontramos dos bloques de código: El primero en las líneas 27-29
...
Si miras el documento compilado, verás que, para el primer bloque se muestra el código y a continuación la salida o resultado de su ejecución; mientras que para el segundo, se muestra solo la salida, y no el código debido a la opción \texttt{echo=FALSE}.
...
El primer bloque de código no tiene ninguna opción. Y el segundo tiene la opción \texttt{echo=TRUE}, que como hemos dicho antes inhibe la impresión del código en el documento compilado.
...
\begin{itemize}
\item
  En un script, se espera que escribamos código \textsf{R}, y para escribir texto ordinario hemos de usar comentario utilizando el carácter \texttt{\#}
\item
  Por el contrario, en un documento R Markdown, se espera que escribamos texto (con posibilidad de incluir la sintaxis propia del lenguaje Markdown), y para escribir código \textsf{R} hemos de incluirlo en un bloque especial delimitado por \texttt{\textasciigrave{}\textasciigrave{}\textasciigrave{}\{r\}} y \texttt{\textasciigrave{}\textasciigrave{}\textasciigrave{}}.
\end{itemize}

\hypertarget{plantilla-de-r-markdown}{%
\section{Plantilla de R Markdown}\label{plantilla-de-r-markdown}}

Ahora vamos a reemplazar los contenidos de muestra del archivo de R Markdown que hemos creado por nuestros propios contenidos. Crearemos un primer capítulo con un primer bloque código y dejaremos el documento preparado para practicar el código de \textsf{R} que se presenta en los siguientes capítulos.
...
Si prefieres que se muestre primero el código para las dos instrucciones y a continuación las dos salidas, añade la opción \texttt{results=\textquotesingle{}hold\textquotesingle{}}:

\begin{Shaded}
\begin{Highlighting}[]
\InformationTok{\textasciigrave{}\textasciigrave{}\textasciigrave{}\{r, results=\textquotesingle{}hold\textquotesingle{}\}}
\InformationTok{sqrt(8)}
\InformationTok{sqrt(10)}
\InformationTok{\textasciigrave{}\textasciigrave{}\textasciigrave{}}
\end{Highlighting}
...
Si quieres omitir los caracteres \texttt{\#\#} al comienzo de las líneas de la salida, añade la opción \texttt{comment\ =\ \textquotesingle{}\textquotesingle{}}.
...
Las opciones especificadas en el argumento de \texttt{knitr::opts\_chunk\$set} aplicarán a todos los bloques de código que se incluyan en el documento.

Si se desea aplicar las opciones \texttt{results=\textquotesingle{}hold\textquotesingle{}} y \texttt{comment\ =\ \textquotesingle{}\textquotesingle{}} que se explicaron antes, a todos los bloques de código del documento, las añadiremos al argumento de la función \texttt{knitr::opts\_chunk\$set}, de forma que quede:
...
\hypertarget{ejecuciuxf3n-individual-de-un-bloque-de-cuxf3digo}{%
\subsection{Ejecución individual de un bloque de código}\label{ejecuciuxf3n-individual-de-un-bloque-de-cuxf3digo}}

Cuando compilamos un documento R Markdown, se ejecutan todos los bloques de código que contenga, y en el documento compilado podemos visualizar, para cada bloque, el código y la salida (si no hay opciones como \texttt{echo=FALSE} que inhiban la impresión del código y/o de la salida).
...
La salida se mostrará en la consola, y también incrustada en el propio documento, justo debajo del bloque de código. Para esto último ha de estar marcada la opción \textbf{Show output inline for all R Markdown documents} en las opciones para R Markdown, como muestra la siguiente imagen:
...
Hasta que el documento no tenga la extensión suficiente para ocupar la pantalla completa y que aparezca la barra de \emph{scroll} para recorrerlo, no se apreciará la funcionalidad completa de la tabla de contenidos flotante.
...
\subsection{Flujo de trabajo}\label{flujo-de-trabajo}}
...
Puedes ir compilando cada bloque o instrucción de forma individual, para ver su salida \emph{inline} y compilar el documento cada cierto tiempo.
...
\hypertarget{objects}{%
\section{Tipos de objetos en R}\label{objects}}

TODO

\hypertarget{plots}{%
\section{Gráficos}\label{plots}}
