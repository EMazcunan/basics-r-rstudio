...
* En el texto podremos utilizar la sintaxis propia del lenguaje de marcado [Markdown](https://es.wikipedia.org/wiki/Markdown) (independiente de `r R()`). Por ejemplo: 
```md
...
Los elementos más importantes del documento son los bloques de código `r R()` (*code chunks*). En el documento de muestra encontramos dos bloques de código: El primero en las líneas 27-29
````md
```{r cars}`r ''`
summary(cars)
```
...
* En un script, se espera que escribamos código `r R()`, y para escribir texto ordinario hemos de usar comentario utilizando el carácter `#`

* Por el contrario, en un documento R Markdown, se espera que escribamos texto (con posibilidad de incluir la sintaxis propia del lenguaje Markdown), y para escribir código `r R()` hemos de incluirlo en un bloque especial delimitado por `  ```{r}  ` y ` ``` `.
