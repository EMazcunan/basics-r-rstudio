...
* En el texto podremos utilizar la sintaxis propia del lenguaje de marcado [Markdown](https://es.wikipedia.org/wiki/Markdown){target="_blank"} (independiente de `r R()`). Por ejempl, para escribir texto negrita se escribe `**` al comienzo y al final del texto en cuestión: 
```md
...
Al presionar el botón **OK** se abrirá una nueva pestaña en el panel de `r RStudio()` con el nuevo documento R Markdown. Presiona `Ctrl + S` para guardarlo, en la carpeta `IntroR` que creaste antes para la práctica, con el nombre `intro-r`. Verifica que la etiqueta de la pestaña del documento cambia de `Untitled1` a `intro-r.Rmd`.
